...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # === Улучшенный Target ===
    future_bars = 6  # ~30 минут вперёд
    df["future_return"] = df["close"].shift(-future_bars) / df["close"] - 1
    df["target"] = (df["future_return"] > 0.0018).astype(int)  # > 0.18%
...
class MLEngine:
    def __init__(self):
        if os.path.exists(MODEL_PATH):
            ...
            self.model = None
            print("[ML] No model found. Train it first.")
    ...
    def predict(self, symbol: str) -> dict:
        ...
        direction = "LONG" if prob > 0.58 else "SHORT"
        ...
        return {
            ...
            "volume_ratio": round(float(last["volume_ratio"]), 3),
        }
